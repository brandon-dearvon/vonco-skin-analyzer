...
class ServerContractTests(unittest.TestCase):
    ...
    def test_complete_result_fills_unreturned_categories_as_unable_to_assess(self) -> None:
        ...
        self.assertIn("needs a clearer photo", by_id["pore_visibility"]["description"])
    ...
    def test_hero_preserves_consumer_value_and_brand_cta_typography(self) -> None:
        ...
        try:
            ...
            self.assertIn("services and skincare thoughtfully matched to what your photos show", html)
            self.assertIn("Full visible profile", html)
            self.assertIn("What looks strong", html)
            self.assertIn("Your clearest focus", html)
            self.assertIn("Matched service + skincare options", html)
            self.assertIn("Your Von &amp; Co options", html)
    ...
    def test_complete_result_shows_summary_full_ordinal_profile_and_direct_action(self) -> None:
        ...
        try:
            ...
            self.assertIn("Your skin at a glance", html)
            ...
            self.assertIn("Clearest focus", html)
            self.assertIn("Photo strength", helper)
            self.assertIn("What already looks strong", html + helper)
            self.assertNotIn("Not apparent", html + helper)
            self.assertNotIn("Profile coverage", html)
            self.assertIn("Book your complimentary consultation", html)
            self.assertLess(html.index('id="prioritiesSection"'), html.index('id="strengthsSection"'))

            self.assertIn('id="profileSection"', html)
            self.assertIn('id="profileList"', html)
            self.assertIn("Your complete photo profile", html)
            self.assertIn("Review every category, from most noticeable to minimal.", html)
    ...
    def test_recommendation_ui_uses_server_catalog_and_safe_dom_rendering(self) -> None:
        ...
        try:
            ...
            self.assertIn('"In-studio options (" + serviceCount', html)
            self.assertIn('"Skincare options (" + productCount', html)
            self.assertIn("Von &amp; Co services linked", html)
            self.assertIn("Skincare options linked", html)
            ...
            self.assertIn("First to explore", html)
            self.assertIn("Also worth exploring", html)
            self.assertIn("First at-home step", html)
            self.assertIn("For maintaining balance", html)
            self.assertIn("look soft and understated in these photos", helper)
            self.assertNotIn("did not stand out", html + helper)
            ...
            self.assertIn("Thoughtful ways to maintain", helper)
            self.assertIn("priorityIds: priorityIds", helper)
            self.assertIn("renderObservationGroup(summary.priorityIds", html)
            self.assertIn("Daily essential", html)
            self.assertNotIn("Why this matches:", html)
            self.assertNotIn("source-supported", html + helper)
            self.assertNotIn("Every current", html + helper)
            self.assertIn("Book your complimentary consultation", html)
