...
This application is **not clinically validated** and must not be described as diagnostic, VISIA-equivalent, or able to rule out skin disease. The production claim is limited to an AI-generated preview of visible surface features in consumer photographs. Treatment names, when shown, are topics to discuss with a qualified Von & Co provider rather than individualized recommendations.
...
- deployed model, prompt hash, response schema, and discussion-topic mapping versions recorded with every result;
...
- server-side observation and discussion-topic mappings;
